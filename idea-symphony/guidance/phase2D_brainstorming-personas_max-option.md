...
### Max Effort (9–12 personas per topic)

1. Start with core: Devil's Advocate + Pragmatist
2. Classify topic type (see Topic Affinity Matrix)
3. Select 2 from "Strong Fit" (same as medium — inner ring)
4. Identify every validated triad/pair with at least one member among
   the 4 selected so far.
5. For each such cluster, add the missing members to complete it:
   - Challenge Triad (DA core) → add Provocateur + Constraint Flipper
   - Human-Centered Triad → complete the missing 1–2 if any of
     Empath / Audience Advocate / Storyteller is selected
   - Analytical Core (Pragmatist core) → add FPT + Technical Expert
     ONLY when topic is genuinely technical
   - External Perspectives Triad → complete the missing 1–2 if any of
     Systems Thinker / Connector / Analogist is selected
   - Forward-Looking Pair → include both if either is justified
6. Check for outer ring candidates:
   - Does the topic have regulatory dimensions? → Consider Lawyer
   - Does the topic have financial dimensions? → Consider Accountant
   - Does the topic have governance/stakeholder dynamics? → Consider Politician
   - Only add outer ring personas if genuinely relevant (see Anti-Patterns)
7. Cap at 12 per topic; drop the lowest-affinity cluster completion if over
