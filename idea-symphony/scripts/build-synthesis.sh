...
shopt -s nullglob
files=(synthesis/[0-9]*.md)
shopt -u nullglob

if [[ ${#files[@]} -eq 0 ]]; then
  echo "Error: no synthesis/[0-9]*.md files found" >&2
  exit 1
fi

# Pull the shared session-level frontmatter values from the first topic
# file (project-name, session-dir, date, effort are identical across all
# per-topic files in a session; only stage and topic-cluster vary).
first="${files[0]}"
