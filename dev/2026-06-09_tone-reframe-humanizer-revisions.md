...
Each must be resolved before plan execution. Recommendations are mine; the user decides.

### Resolutions (2026-06-09)

All ten resolved in a walkthrough with the user. Three rested on code facts that were verified first (OQ1, OQ2, OQ9).

1. **OQ1 — Fix 1 scope: med/high only.** Verified the categorical-reframe instruction lives only in `phase4_full-synthesis.md`; both summary-only prompts carry none. State this in the Fix 1 implementation notes so the executor doesn't search min/low.
2. **OQ2 — Fix 3 gating confirmed:** `_summary.md` all effort levels, `_synthesis.md` med/high only, `BRAINSTORM.md` all levels (matches SKILL.md:354-360).
3. **OQ3 — Hybrid wiring:** Phase 5 inline (a), Phase 4 separate post-step (b).
4. **OQ4 — Embedded humanizer:** use `idea-symphony/prompts/humanizer/SKILL.md` (v2.5.1, confirmed present). **Committed 2026-06-09 (`9b056b4`)** — vendored ahead of Fix 3 so it ships with the skill (was previously untracked).
5. **OQ5 — Retry policy:** cap at 2, then fall back to the un-humanized original block and log in PLAN.md Notes (supersedes the earlier "escalate to a human" wording — never blocks an unattended run). Granularity: per-`### Question N` block for `_synthesis.md`, whole-file for `_summary.md`.
6. **OQ6 — Bulk regen:** land this revision first; its step 9 regen absorbs the final-polish bulk regen.
7. **OQ7 — `BRAINSTORM_humanized.md`:** keep as-is in `test-runs/250T/`.
8. **OQ8 — Canonical NLM skill:** out of scope. **Follow-up completed 2026-06-09** — both `nlm-audio-prompt` and `nlm-slide-deck` audited (spot-verified) and found clean on all three Fix-5 patterns; no canonical-side edits warranted. Record: [2026-06-09_canonical-nlm-skills-audit.md](2026-06-09_canonical-nlm-skills-audit.md).
9. **OQ9 — Coverage:** complete. `SYNTHESIS.md`/`SUMMARIES.md` inherit humanization. New requirement: re-sequence `build-summaries.sh`/`build-synthesis.sh` to run *after* the Fix 3 Phase 4 humanizer post-step (they currently run at SKILL.md:343, before it).
10. **OQ10 — Superseded files:** the `2026-05-15` plan is untracked (git does *not* already preserve it), so commit it and this consolidated doc first, then delete the `2026-05-15` plan and the `~/.claude/plans/…floating-whistle.md` scratch.

### OQ1: Effort-conditional scope of Fix 1 (reframe demotion)

Min and low effort use different Phase 4 prompts ([`phase4_summary-only_*`](idea-symphony/prompts/)) that don't include the categorical-reframe instruction. Med and high share `phase4_full-synthesis.md`. The fix only needs to apply to med/high. Aligns with the [two-regime evaluation lens](2026-05-03_effort-comparison.md) §5.

**Recommendation.** Confirm: yes, Fix 1 is med/high-only. The min/low paths don't carry the bug. No prompt changes needed at min/low. State this explicitly in the Fix 1 implementation notes so the executor doesn't search for nonexistent code paths.

### OQ2: Effort-conditional scope of Fix 3 (humanizer post-pass)

Per `idea-symphony/SKILL.md` lines 355–360, `_synthesis.md` is only emitted at medium/high effort. `_summary.md` exists at all effort levels (min/low/medium/high). `BRAINSTORM.md` exists at all effort levels.

**Recommendation.** Confirm:
- `_summary.md` humanizer pass: all effort levels.
- `_synthesis.md` humanizer pass (chunk-per-question): medium/high only.
- `BRAINSTORM.md` humanizer pass: all effort levels.

This is the natural reading and matches what files exist where; just needs to be explicit so the SKILL.md routing is unambiguous.

### OQ3: Inline vs separate humanizer pass (orchestration choice)

Two integration options for Fix 3 wiring:
- (a) Inline at the end of each Phase 4 cluster subagent and the Phase 5 subagent. Each subagent loads the humanizer skill.
- (b) Separate post-step in SKILL.md routing, fanning out one humanizer subagent per `_summary.md` file and one per `### Question N` block of each `_synthesis.md`, with reassembly. Adds an orchestration step but keeps each subagent focused.

**Recommendation.** **(a) for Phase 5** (single subagent, one file — inline is simplest). **(b) for Phase 4** (already a fan-out structure at medium/high; wrapping one more pass around the existing fan-out is mechanically simpler than threading humanizer logic into the synthesis subagent, and (b) is needed anyway for the `_synthesis.md` per-question fan-out).

### OQ4: Humanizer skill location reference

Fix 3 brief references `{{skill}}/prompts/humanizer/SKILL.md` (i.e. `idea-symphony/prompts/humanizer/SKILL.md`). There is also a canonical Anthropic humanizer skill (referenced in the available-skills list as `anthropic-skills:humanizer`). The embedded Symphony copy and the canonical one are not currently linked or version-checked.

**Recommendation.** Use the embedded `idea-symphony/prompts/humanizer/SKILL.md` for Fix 3. It is already in scope, has no external dependency, and ships with the skill. If the canonical skill drifts ahead, sync the embedded copy as a separate maintenance task.

### OQ5: Handling of axis-K spot-read flags in production

Round 2 surfaced ~7% editorial drift in one of five parallel Haiku agents (bold-lead rewriting and one prefix drop). Axis K's spot-read will flag these at test time and presumably at runtime too. The plan calls for regeneration of affected blocks but doesn't specify the operational mechanism.

**Recommendation.** When axis K spot-read flags an issue:
- For `_synthesis.md` (chunked): regenerate only the affected `### Question N` block(s), not the whole file. The chunked wiring already produces per-question artifacts, so this is mechanically free.
- For `_summary.md` (single-call): regenerate the whole file.
- Cap retries at 2 — if the same block fails axis K twice, **fall back to the un-humanized original block** (its content is already correct; only minor surface tells remain) and log it in PLAN.md Notes, rather than burning Haiku calls on a stuck pattern or blocking an unattended run on a human. *(Resolved 2026-06-09: fall-back-to-original adopted over the earlier escalate-to-human option.)*

### OQ6: Bulk regen ordering vs the final-polish plan's bulk regen

The [final-polish plan](2026-05-09_symphony-final-polish.md) §3–4 calls for bulk regen of 6 sessions. If this revision lands before the final-polish bulk regen kicks off, those 6 regenerations will use the revised prompts. If bulk regen has already started, we'd want to either pause it or accept a re-regen on the revised prompts after.

**Recommendation.** Land this revision (steps 2–7 of the Sequence) before the final-polish bulk regen. The final-polish bulk regen becomes the bulk regen of step 9 here — they collapse into the same operation, and the test-run matrix gets the unified post-revision outputs.

### OQ7: `BRAINSTORM_humanized.md` (250T) — canonize as reference?

The hand-humanized version of 250T's BRAINSTORM is a useful before/after artifact and serves as a target for what the humanizer post-pass should produce.

**Recommendation.** Keep in `test-runs/250T/` (current location). Don't promote it to anything more official, don't delete it. It's serving its purpose as a test-runs artifact and as a reference for axis L's "more natural than the pre-humanizer version" judgment.

### OQ8: Should Fix 5 also tighten the canonical `nlm-audio-prompt` skill?

The current Symphony template explicitly mirrors that skill's modules (see lines 11–13 of [`notebook-lm-instructions.md`](idea-symphony/templates/notebook-lm-instructions.md)). Symphony adds the Source Framing module on top. The de-jargon edits in Fix 5 are Symphony-specific (the canonical skill is consumed with arbitrary sources, not Symphony output). But if the canonical skill itself has analogous jargon issues, that's a separate revision worth scoping.

**Recommendation.** Out of scope for this revision. Track as a separate follow-up item ("audit canonical `nlm-audio-prompt` skill for the same jargon patterns Fix 5 fixes in the Symphony template"). The two skills are owned differently and shipping them as one revision conflates Symphony's release cycle with the canonical skill's.

**Resolved 2026-06-09:** follow-up done — `nlm-audio-prompt` and `nlm-slide-deck` audited and confirmed clean (spot-verified); no canonical edits warranted. Two optional micro-hardenings and one Symphony-side integration opportunity recorded in [2026-06-09_canonical-nlm-skills-audit.md](2026-06-09_canonical-nlm-skills-audit.md).

### OQ9: Are there other places `_synthesis.md` should be humanized that we missed?

The scope analysis covered Phase 4's three documents (`attributed/`, `_synthesis.md`, `_summary.md`) and Phase 5's `BRAINSTORM.md`. The downstream NLM addon output is explicitly out of scope (machine-readable). Are there other generated files?

**Recommendation.** Per [`idea-symphony/SKILL.md`](idea-symphony/SKILL.md) lines 343, 355–360, the artifacts at medium/high effort are: `attributed/`, `_synthesis.md`, `_summary.md`, `SYNTHESIS.md` (concat), `SUMMARIES.md` (concat), `BRAINSTORM.md`, and optionally `NOTEBOOK-LM-INSTRUCTIONS.md`. The two concatenated files (`SYNTHESIS.md`, `SUMMARIES.md`) are produced by deterministic shell scripts from the per-cluster files and don't need a separate humanizer pass — they inherit humanization from their substrate. **Resolved 2026-06-09:** confirmed — `build-summaries.sh`/`build-synthesis.sh` are deterministic concat-and-strip-frontmatter transforms (SKILL.md:343), so the concatenated files inherit humanization. **Sequencing requirement:** they must run *after* the Fix 3 Phase 4 humanizer post-step; SKILL.md:343 currently runs them immediately after the synthesis subagents, which would otherwise concatenate un-humanized substrate.

### OQ10: Disposition of the two superseded planning files

The merge into this consolidated doc supersedes two earlier files:
- `dev/2026-05-15_tone-reframe-humanizer-revisions.md` (the original revisions plan)
- `~/.claude/plans/let-s-see-what-happens-floating-whistle.md` (the plan-mode scratch with the `_synthesis.md` discussion and verification rounds)

**Recommendation.** Delete both after a human confirms this consolidated doc is complete. **Correction (2026-06-09):** the `2026-05-15` plan is untracked with no commit history — git does *not* already preserve it, so it (and this consolidated doc) must be committed *before* deletion. The `~/.claude/plans/` file is local-only scratch and has no archival value.
