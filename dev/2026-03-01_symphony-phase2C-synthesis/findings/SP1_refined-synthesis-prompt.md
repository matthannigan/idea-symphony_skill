# Variant S-V2-iter3 — Tightened High-Effort Preserve Gate, Strengthened Append Template, Source-Bound ST Exception, Medium-Effort Cluster Cap

**Base:** S-V2-iter2 (iter2 refined prompt; R1–R8 applied).
**Iteration:** 3 of 3 (refinements R9–R12 applied; R1–R8 retained).
**Changes:** targeted fixes for iter2's remaining failures — R9 tightens R5's `preserve-distinct-≥4` exception at high effort only (fixes SC/high regression and high-effort under-compaction); R10 strengthens R7's Append template with a cluster-proximity diagnostic (lifts low-effort Append fidelity); R11 adds source-bound ST exception handling (graceful MA/low classification per iter2 finding); R12 adds a medium-effort cluster-count ceiling for low-N_synth topics (fixes FT/med over-clustering). Preserves S-V2 Synthesize/Append split and iter1/iter2 per-effort compaction targets. No rollback of R1/R2/R3/R4/R6/R7; R5 tightened at high only; R8 retained with R11 source-bound carve-out.
